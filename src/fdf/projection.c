...
/*   Updated: 2025/02/24 18:01:05 by fvon-de          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_point apply_projection(t_renderer *renderer, t_point point, float scale) {
	t_point projected;
	float temp_x;
	float temp_y;
	...
	if (renderer->camera->iso == 1) {
		...
		projected.y = ((point.x + point.y) * scale * 0.7071f) - (point.z * scale * renderer->camera->z_scale);
